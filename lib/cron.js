...
function getInstalledVersion() {
  const pkgPath = path.join(process.cwd(), 'node_modules', 'gigabot', 'package.json');
...
async function fetchAndStoreReleaseNotes(target) {
  try {
    const ghRes = await fetch(
      `https://api.github.com/repos/gignaati/gigabot/releases/tags/v${target}`
    );
    if (!ghRes.ok) return;
...
/**
 * Check npm registry for a newer version of gigabot.
 */
  ...
  try {
    ...
    if (isPrerelease(installed)) {
      ...
      const results = await Promise.allSettled([
        fetch('https://registry.npmjs.org/gigabot/latest'),
        fetch('https://registry.npmjs.org/gigabot/beta'),
      ]);
      ...
      const res = await fetch('https://registry.npmjs.org/gigabot/latest');
      if (!res.ok) {
