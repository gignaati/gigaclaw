...
const KEY_PREFIX = 'gcl_';

// In-memory cache: { key_hash, id } or null
let _cache = null;

/**
 * Generate a new API key: gcl_ + 64 hex chars (32 random bytes).
 * @returns {string}
 */
...
export function createApiKeyRecord(createdBy) {
  ...
  const keyPrefix = key.slice(0, 8); // "gcl_" + first 4 hex chars
  const now = Date.now();
...
export function verifyApiKey(rawKey) {
  // Accept both current (gcl_) and legacy (tpb_) prefixes for backward compatibility
  if (!rawKey || (!rawKey.startsWith(KEY_PREFIX) && !rawKey.startsWith('tpb_'))) return null;
