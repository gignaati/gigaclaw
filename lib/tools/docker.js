...
async function detectNetwork() {
  try {
    const { status, data } = await dockerApi('GET', '/containers/gigabot-event-handler/json');
...
async function createCodeWorkspaceContainer({ containerName, repo, branch, codingAgent = 'claude-code' }) {
  ...
  const version = process.env.GIGABOT_VERSION || 'latest';
  const image = `gignaati/gigabot:claude-code-workspace-${version}`;
  ...
  if (inspectRes.status !== 200) {
    const pullRes = await dockerApi('POST', `/images/create?fromImage=${encodeURIComponent('gignaati/gigabot')}&tag=${encodeURIComponent(`claude-code-workspace-${version}`)}`);
