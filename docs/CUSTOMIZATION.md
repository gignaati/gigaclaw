...
npx gigaclaw set-agent-secret GH_TOKEN ghp_xxx
npx gigaclaw set-agent-secret ANTHROPIC_API_KEY sk-ant-xxx

# Accessible to LLM (not filtered) — set via CLI:
npx gigaclaw set-agent-llm-secret BROWSER_PASSWORD mypass123
```
