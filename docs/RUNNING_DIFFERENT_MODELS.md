...
gigaclaw has two layers that use LLMs independently:
...
```bash
npx gigaclaw set-var LLM_PROVIDER openai
npx gigaclaw set-var LLM_MODEL gpt-4o
```

The matching API key must exist as a GitHub secret:

```bash
npx gigaclaw set-agent-secret OPENAI_API_KEY sk-...
```
...
```bash
npx gigaclaw set-var LLM_PROVIDER custom
npx gigaclaw set-var LLM_MODEL qwen3:8b
npx gigaclaw set-var OPENAI_BASE_URL http://host.docker.internal:11434/v1
npx gigaclaw set-var RUNS_ON self-hosted
```
...
```bash
npx gigaclaw set-var LLM_PROVIDER custom
npx gigaclaw set-var LLM_MODEL deepseek-chat
npx gigaclaw set-var OPENAI_BASE_URL https://api.deepseek.com/v1
```

Then set the API key as a GitHub secret:

```bash
npx gigaclaw set-agent-secret CUSTOM_API_KEY sk-...
```
...
| Default job model | GitHub repo variables | `LLM_PROVIDER`, `LLM_MODEL` (set via `npx gigaclaw set-var`) |
