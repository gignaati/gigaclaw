...
| `DATABASE_PATH` | Override SQLite database location (default: `data/gigabot.sqlite`) | No |
| `GIGABOT_VERSION` | Package version for Docker image tags (auto-set by setup) | No |
...
Set via `npx gigabot set-agent-secret` and `npx gigabot set-agent-llm-secret`. Each value is stored exactly as provided.
...
| `JOB_IMAGE_URL` | Docker image path for job agent (e.g., `ghcr.io/myorg/mybot`) | No | `gignaati/gigabot:pi-coding-agent-job-${GIGABOT_VERSION}` |
| `EVENT_HANDLER_IMAGE_URL` | Docker image path for event handler | No | `gignaati/gigabot:event-handler-${GIGABOT_VERSION}` |
