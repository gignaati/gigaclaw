...
| `DATABASE_PATH` | Override SQLite database location (default: `data/gigaclaw.sqlite`) | No |
| `GIGACLAW_VERSION` | Package version for Docker image tags (auto-set by setup) | No |
...
Set via `npx gigaclaw set-agent-secret` and `npx gigaclaw set-agent-llm-secret`. Each value is stored exactly as provided.
...
| `JOB_IMAGE_URL` | Docker image path for job agent (e.g., `ghcr.io/myorg/mybot`) | No | `gignaati/gigaclaw:pi-coding-agent-job-${GIGACLAW_VERSION}` |
| `EVENT_HANDLER_IMAGE_URL` | Docker image path for event handler | No | `gignaati/gigaclaw:event-handler-${GIGACLAW_VERSION}` |
